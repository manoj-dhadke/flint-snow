# begin
@log.trace("Started executing 'serviceNow:problem:assigned_to.rb' flintbit...")
begin
    ...
    data = {}
    data["assigned_to"]=@input.get('assigned_to')
...
state :: #{data}")
  ...
  response_body = response.get('body')# Response body
    ...
    if response_exitcode == 0
        ...
        response= @call.bit("serviceNow:problem:get_process_data.rb").setraw(response_body).sync
        #@output.setraw('result', response_body)
        @output.set('result', response.get('data'))
        @log.info("Sucessfully Assigned To :: #{data["assigned_to"]}")
...
@log.trace("Finished executing 'serviceNow:problem:assigned_to.rb' flintbit...")
# end
