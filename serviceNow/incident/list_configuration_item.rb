# begin
@log.trace("Started executing 'flint-util:serviceNow:incident:list_configuration_list.rb' flintbit...")
begin
  ...
  #@log.info(response_body) 
    ...
    if response_exitcode == 0
	 ...
	 @log.info("Success in executing serviceNow Connector, where exitcode :: #{response_exitcode} | message :: #{response_message}")
         #@log.info("#{result}")
         #response = @call.bit("serviceNow:incident:process_data.rb").setraw(response_body).sync  
...
@log.trace("Finished executing 'flint-util:serviceNow:incident:list_configuration_list.rb' flintbit...")
# end
