# begin
@log.trace("Started executing 'serviceNow:post.rb' flintbit...")
begin
    # Flintbit Input Parameters
    # mandatory
    connector_name = @input.get('connector_name') 		# Name of the HTTP Connector
    request_method = 'post'  
   # request_url =''                 		# HTTP Request Method
    a = @config.global("flintserve.protocol")
    b = @config.global("flintserve.hostname")
    c = @config.global("flintserve.port")

    request_url = a+'://'+b+':'+c.to_s+'/serviceNowIncident/syncIncident'              # HTTP Request URL
...
@log.trace("Finished execution 'serviceNow:post.rb' flintbit...")
 #end
