...
# begin
@log.trace("Started executing 'flint-snow:change:list.rb' flintbit...")
begin
    ...
    sysparm_fields = "sys_id, number, closed_at, priority, incident_state, assigned_to, category ,correlation_id, sys_updated_by, sys_updated_on ,caller_id, assignment_group, short_description, description, cmdb_ci, state, opened_at, start_date, end_date"
  ...
  response_body = response.get('body')# Response body   
    ...
    if response_exitcode == 0
	 ...
	 @log.info("Success in executing serviceNow Connector, where exitcode :: #{response_exitcode} | message :: #{response_message}")
        # @log.info("#{result}")
         response = @call.bit("flint-snow:change:process_data.rb").setraw(response_body).sync        
...
@log.trace("Finished executing 'flint-snow:change:list.rb' flintbit...")
# end
